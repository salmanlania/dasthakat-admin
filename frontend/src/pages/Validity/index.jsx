...
const Validity = () => {
  ...
  const { list, params, deleteIDs } = useSelector((state) => state.validity);
  ...
  const onSubmit = (id, value, prevValue) => {
    if (id === "new") {
      return dispatch(removeNewValidity());
    }

    // if (value === prevValue || !value) {
    //   return dispatch(setValidityEditable({ key: id, editable: false }));
    // }
      ...
      render: (_, { name, editable, validity_id }) =>
        editable ? (
          <Input
            autoFocus
            defaultValue={name}
            onPressEnter={(e) => onSubmit(validity_id, e.target.value, name)}
            onBlur={(e) => onSubmit(validity_id, e.target.value, name)}
          />
        ) : (
          <span>{name}</span>
        ),
