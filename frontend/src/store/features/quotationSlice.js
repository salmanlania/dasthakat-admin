  ...
  extraReducers: ({ addCase }) => {
    ...
    addCase(getQuotation.fulfilled, (state, action) => {
      ...
      state.quotationDetails = data.quotation_detail.map((detail) => ({
        ...
        cost_price:
          detail?.product_type?.product_type_id === 4
            ? (detail?.cost_price === 0 && detail?.last_valid_rate
              ? detail?.last_valid_rate
              : !detail?.last_valid_rate && detail?.cost_price
                ? detail?.cost_price
                : detail?.cost_price || 0)
            : +detail?.cost_price || +detail?.rate,
        markup: detail.markup,
        rate: detail?.rate ? detail?.rate : 0,
        // rate: detail?.cost_price === 0 && detail?.last_valid_rate ? detail?.last_valid_rate : !detail?.last_valid_rate && detail?.cost_price ? detail?.cost_price : detail?.cost_price || 0,
      ...
      state.vendorQuotationDetails = data.quotation_detail
        .map((detail) => ({
          ...
          markup: detail.markup,
          rate: detail?.rate ? detail?.rate : 0,
          last_valid_rate: detail?.last_valid_rate ? detail?.last_valid_rate : 0,
      ...
      state.quotationDetails = data.quotation_detail.map((detail) => ({
        ...
        cost_price:
          detail?.product_type?.product_type_id === 4
            ...
            : +detail.cost_price || +detail.rate,
        markup: detail.markup,
        rate: detail?.rate < 0 ? detail?.last_valid_rate : detail?.rate > 0 ? detail?.rate : 0,
        full_rate: detail.full_rate,
        old_rate: detail.old_rate,
        amount: detail.amount,
        discount_percent: detail.discount_percent,
