...
export const getServiceOrderList = createAsyncThunk(
  'service_order/list',
  async (params, { rejectWithValue }) => {
    try {
      const res = await api.get('/service_order', {
        params: {
          ...params,
          all: 1
        }
...
export const deleteServiceOrder = createAsyncThunk(
  ...
  async (id, { rejectWithValue }) => {
    try {
      await api.delete(`/service_order/${id}`);
    } catch (err) {
      throw rejectWithValue(err);
    }
  }
...
export const bulkDeleteServiceOrder = createAsyncThunk(
  'service_order/bulkDelete',
...
export const shipmentSlice = createSlice({
  ...
  initialState,
  reducers: {
    setServiceOrderListParams: (state, action) => {
      state.params = {
        ...state.params,
        ...action.payload
      };
    ...
    setServiceOrderDeleteIDs: (state, action) => {
  ...
  extraReducers: ({ addCase }) => {
    addCase(getServiceOrderList.pending, (state) => {
      state.chargeOrderDetails = [];
      state.isListLoading = true;
      state.initialFormValues = null;
    });
    addCase(getServiceOrderList.fulfilled, (state, action) => {
    ...
    addCase(getServiceOrderList.rejected, (state) => {
      state.isListLoading = false;
    });
    ...
    addCase(bulkDeleteServiceOrder.pending, (state) => {
      state.isBulkDeleting = true;
    });
    addCase(bulkDeleteServiceOrder.fulfilled, (state) => {
      state.isBulkDeleting = false;
      state.deleteIDs = [];
    });
    addCase(bulkDeleteServiceOrder.rejected, (state) => {
      state.isBulkDeleting = false;
    });
  }
});

export const { setServiceOrderListParams, setServiceOrderDeleteIDs, setChargeOrderDetails } =
