...
const PurchaseOrderForm = ({ mode, onSubmit }) => {
  ...
  const onFinish = (values) => {
    ...
    const data = {
      ...
      purchaseOrder_detail: purchaseOrderDetails.map(
        ({ id, ...detail }, index) => ({
          ...detail,
        ...
        return (
          <div className="flex flex-col gap-1">
            <Button
              ...
              onClick={() => {
                dispatch(
                  changePurchaseOrderDetailOrder({ from: index, to: index - 1 })
                );
              }}
            />
            <Button
      ...
      <Row gutter={12}>
        <Col span={24} sm={12} md={8} lg={8}>
          <Form.Item
            ...
            rules={[
              { required: true, message: "Purchase Order date is required" },
            ]}
            className="w-full"
          >
