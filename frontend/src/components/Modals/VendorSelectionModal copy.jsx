// import { Modal, Table, Input, Spin, Radio, Button, Switch, DatePicker } from 'antd';
...
//       required_date: requiredDate ? dayjs(requiredDate).format('YYYY-MM-DD') : '',
//       quotation_detail: [],
//     };
...
//       value === 'exit' ? onClose() : await dispatch(getVendor(id)).unwrap();
//     } catch (error) {
//       handleError(error);
//     } finally {
//       setIsSaving(false);
...
//           return (
//             <div style={{ whiteSpace: 'normal', wordWrap: 'break-word', maxWidth: 200 }}>
//               <AsyncSelect
//                 endpoint="/supplier"
...
//                 className="w-full"
//                 disabled={
//                   record.product_type_id?.value === 1 || record.product_type_id?.value === 2
//                 }
...
//           type="primary"
//           onClick={() => onFinish('exit')}
//           loading={isExitSaving}>
//           Save & Exit
//         </Button>
//       </div>
//     </Modal>
...
const VendorQuotationForm = ({ mode, onSubmit, onSave }) => {
  ...
  const onFinish = (values) => {
    ...
    const data = {
      ...
      required_by: values.required_by ? dayjs(values.required_by).format('YYYY-MM-DD HH:mm') : null,
      returned_on: values.returned_on ? dayjs(values.returned_on).format('YYYY-MM-DD HH:mm') : null,
      attn: values.attn,
        ...
        initialValues={
          mode === 'edit'
            ? {
                ...
                required_by: initialFormValues.required_by
                  ? dayjs(initialFormValues.required_by)
                  : null,
                returned_on: initialFormValues.returned_on
                  ? dayjs(initialFormValues.returned_on)
                  : null,
              }
            : {
                document_date: dayjs(),
        ...
        <Row gutter={12}>
          ...
          <Col span={24} sm={12} md={8} lg={8}>
            <Form.Item
              ...
              className="w-full">
              <DatePicker showTime format="MM-DD-YYYY HH:mm" className="w-full" />
            </Form.Item>
          </Col>
          ...
          <Col span={24} sm={12} md={8} lg={8}>
            <Form.Item
              name="returned_on"
              label="Rtn'd On"
              rules={[{ required: true, message: 'Returned on date is required' }]}
              className="w-full">
              <DatePicker showTime format="MM-DD-YYYY HH:mm" className="w-full" />
            </Form.Item>
          </Col>
          ...
          <div className="flex-1 lg:w-2/5">
            <Row gutter={[12, 12]}>
              <Col span={24} sm={12} md={6} lg={6}>
                ...
                />
                <DetailSummaryInfo title="Total:" value={formatThreeDigitCommas(roundUpto(totalCost)) || 0} />
                <DetailSummaryInfo
                  title="Final Total:"
                  value={formatThreeDigitCommas(roundUpto(finalAmount)) || 0}
                />
              </Col>
              <Row gutter={[12, 12]}>
