...
class PurchaseReturnController extends Controller
{
    ...
    public function delete($id, Request $request)
    {
        ...
        try {
            ...
            DB::rollBack();
            return $this->jsonResponse('Error deleting purchase return: ' . $e->getMessage(), 500);
        }
    }
    ...
    public function bulkDelete(Request $request)
    {
        ...
        try {
            ...
            if (empty($ids)) {
                throw new \Exception('No Purchase Return IDs provided');
            }
            ...
            foreach ($purchaseReturns as $purchaseReturn) {
                ...
                StockLedger::where('document_id', $purchaseReturn->purchase_return_id)->delete();
                $deletedCount++;
            }

            return $this->jsonResponse('Deleted', 200, "{$deletedCount} Purchase Return(s) Deleted Successfully!");
        } catch (\Exception $e) {
            return $this->jsonResponse('Error deleting purchase returns: ' . $e->getMessage(), 500);
        }
    }
}