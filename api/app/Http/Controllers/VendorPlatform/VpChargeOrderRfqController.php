...
class VpChargeOrderRfqController extends Controller
{
    protected $db;
    ...
    public function index(Request $request)
    {
        ...
        $data = VpChargeOrderRfq::leftJoin('charge_order as q', 'q.charge_order_id', '=', 'vp_charge_order_rfq.charge_order_id')
        ...
        // Mandatory filters
        $data = $data->where('vp_charge_order_rfq.company_id', '=', $request->company_id);
        $data = $data->where('vp_charge_order_rfq.company_branch_id', '=', $request->company_branch_id);

        // Apply existing filters
        if (!empty($document_identity)) {
            $data = $data->where('vp_charge_order_rfq.document_identity', 'like', '%' . $document_identity . '%');
        }
        if (!empty($charge_order_no)) {
            $data = $data->where('q.document_identity', 'like', '%' . $charge_order_no . '%');
        }
        if (!empty($vessel_id)) {
            $data = $data->where('q.vessel_id', $vessel_id);
        }
        if (!empty($notification_count)) {
            $data = $data->where('vp_charge_order_rfq.notification_count', $notification_count);
        }
        if (!empty($event_id)) {
        ...
        if (!empty($status)) {
            if ($status == 'Cancelled') {
                $data = $data->where('vp_charge_order_rfq.is_cancelled', 1);
            } elseif ($status == 'Bid Sent') {
                $data = $data->whereRaw('vp_charge_order_rfq.is_cancelled = 0 
            AND (SELECT COUNT(*) FROM vp_charge_order_rfq_detail vqd 
                 WHERE vqd.id = vp_charge_order_rfq.id 
                 AND vqd.vendor_rate IS NOT NULL 
                 AND vqd.vendor_rate != "") = 0 
            AND vp_charge_order_rfq.date_required >= ?', [$currentDate]);
            ...
            AND (SELECT COUNT(*) FROM vp_charge_order_rfq_detail vqd 
                 WHERE vqd.id = vp_charge_order_rfq.id 
                 AND vqd.vendor_rate IS NOT NULL 
                 AND vqd.vendor_rate != "") < (SELECT COUNT(*) FROM vp_charge_order_rfq_detail vqd 
                                                WHERE vqd.id = vp_charge_order_rfq.id) 
            AND vp_charge_order_rfq.date_required >= ?', [$currentDate]);
            ...
            AND (SELECT COUNT(*) FROM vp_charge_order_rfq_detail vqd 
                ...
                $data = $data->whereRaw('vp_charge_order_rfq.date_required < ? 
            AND (SELECT COUNT(*) FROM vp_charge_order_rfq_detail vqd 
                 WHERE vqd.id = vp_charge_order_rfq.id 
                 AND vqd.vendor_rate IS NOT NULL 
                 AND vqd.vendor_rate != "") != (SELECT COUNT(*) FROM vp_charge_order_rfq_detail vqd 
                                                 WHERE vqd.id = vp_charge_order_rfq.id)
                                                 AND vp_charge_order_rfq.is_cancelled = 0 ', [$currentDate]);
            }
        }
        ...
        if (!empty($search)) {
            ...
            $data = $data->where(function ($query) use ($search) {
                $query
                    ->where('vp_charge_order_rfq.document_identity', 'like', '%' . $search . '%')
        ...
        if (!empty($date_from) && !empty($date_to)) {
            $data = $data->whereBetween('vp_charge_order_rfq.date_sent', [
                $date_from . ' 00:00:00',
                $date_to . ' 23:59:59'
            ]);
        } elseif (!empty($date_from)) {
            $data = $data->where('vp_charge_order_rfq.date_sent', '>=', $date_from . ' 00:00:00');
        } elseif (!empty($date_to)) {
            $data = $data->where('vp_charge_order_rfq.date_sent', '<=', $date_to . ' 23:59:59');
        }

        // Exact match filters for total_items and items_quoted
        if (is_numeric($total_items) && $total_items >= 0) {
            $data = $data->havingRaw('(SELECT COUNT(*) FROM vp_charge_order_rfq_detail vqd WHERE vqd.id = vp_charge_order_rfq.id) = ?', [$total_items]);
        }
        if (is_numeric($items_quoted) && $items_quoted >= 0) {
            $data = $data->havingRaw('(SELECT COUNT(*) FROM vp_charge_order_rfq_detail vqd WHERE vqd.id = vp_charge_order_rfq.id AND vqd.vendor_rate IS NOT NULL AND vqd.vendor_rate != "") = ?', [$items_quoted]);
        }

        // Status sorting
        if ($sort_column == 'status') {
            $orderByCase = "CASE 
            WHEN vp_charge_order_rfq.is_cancelled = 1 THEN 'Cancelled'
            WHEN (SELECT COUNT(*) FROM vp_charge_order_rfq_detail vqd WHERE vqd.id = vp_charge_order_rfq.id AND vqd.vendor_rate IS NOT NULL AND vqd.vendor_rate != '') = 0 AND vp_charge_order_rfq.date_required >= '$currentDate' THEN 'Bid Sent'
            WHEN (SELECT COUNT(*) FROM vp_charge_order_rfq_detail vqd WHERE vqd.id = vp_charge_order_rfq.id AND vqd.vendor_rate IS NOT NULL AND vqd.vendor_rate != '') > 0 AND (SELECT COUNT(*) FROM vp_charge_order_rfq_detail vqd WHERE vqd.id = vp_charge_order_rfq.id AND vqd.vendor_rate IS NOT NULL AND vqd.vendor_rate != '') < (SELECT COUNT(*) FROM vp_charge_order_rfq_detail vqd WHERE vqd.id = vp_charge_order_rfq.id) AND vp_charge_order_rfq.date_required >= '$currentDate' THEN 'Partial'
            WHEN (SELECT COUNT(*) FROM vp_charge_order_rfq_detail vqd WHERE vqd.id = vp_charge_order_rfq.id AND vqd.vendor_rate IS NOT NULL AND vqd.vendor_rate != '') = (SELECT COUNT(*) FROM vp_charge_order_rfq_detail vqd WHERE vqd.id = vp_charge_order_rfq.id) THEN 'Bid Received'
            WHEN vp_charge_order_rfq.date_required < '$currentDate' THEN 'Bid Expired'
            ELSE '-'
        END";
            ...
            if ($sort_column == 'total_items') {
                $data = $data->orderByRaw('(SELECT COUNT(*) FROM vp_charge_order_rfq_detail vqd WHERE vqd.id = vp_charge_order_rfq.id) ' . $sort_direction);
            } elseif ($sort_column == 'items_quoted') {
                $data = $data->orderByRaw('(SELECT COUNT(*) FROM vp_charge_order_rfq_detail vqd WHERE vqd.id = vp_charge_order_rfq.id AND vqd.vendor_rate IS NOT NULL AND vqd.vendor_rate != "") ' . $sort_direction);
            } else {
        ...
        $data = $data->select(
            'vp_charge_order_rfq.*',
            'q.document_identity as charge_order_no',
            ...
            DB::raw("(SELECT COUNT(*) FROM vp_charge_order_rfq_detail vqd WHERE vqd.id = vp_charge_order_rfq.id AND vqd.vendor_rate IS NOT NULL AND vqd.vendor_rate != '') as items_quoted"),
            DB::raw("CASE 
            WHEN vp_charge_order_rfq.is_cancelled = 1 THEN 'Cancelled'
            WHEN (SELECT COUNT(*) FROM vp_charge_order_rfq_detail vqd WHERE vqd.id = vp_charge_order_rfq.id AND vqd.vendor_rate IS NOT NULL AND vqd.vendor_rate != '') = 0 AND vp_charge_order_rfq.date_required >= '$currentDate' THEN 'Bid Sent'
            WHEN (SELECT COUNT(*) FROM vp_charge_order_rfq_detail vqd WHERE vqd.id = vp_charge_order_rfq.id AND vqd.vendor_rate IS NOT NULL AND vqd.vendor_rate != '') > 0 AND (SELECT COUNT(*) FROM vp_charge_order_rfq_detail vqd WHERE vqd.id = vp_charge_order_rfq.id AND vqd.vendor_rate IS NOT NULL AND vqd.vendor_rate != '') < (SELECT COUNT(*) FROM vp_charge_order_rfq_detail vqd WHERE vqd.id = vp_charge_order_rfq.id) AND vp_charge_order_rfq.date_required >= '$currentDate' THEN 'Partial'
    ...
    {
        if (!isPermission('edit', 'vp_charge_order', $request->permission_list))
        ...
        foreach ($ids as $id) {
            ...
            if ($rfq_reset == 1) {
                ...
                $rfq->is_cancelled = 0;
                VpChargeOrderRfqDetail::where('id', $id)->update([
                    'vendor_rate' => null,
                    'vendor_part_no' => null,
                    'vendor_notes' => null,
                ]);
        ...
        return $this->jsonResponse([], 200, "Vendor Charge Order RFQ updated successfully!");
    }
    ...
    public function sendRFQNotification($id)
    {
        try {
            ...
            $data = VpChargeOrderRfq::with('charge_order', 'vendor')->find($id);
            ...
            $payload = [
                ...
                'subject' => "RFQ Notification for Charge Order ID: {$data->charge_order->document_identity}",
                'message' => '',
            ];
